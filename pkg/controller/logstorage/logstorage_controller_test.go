			...
			Context("LogStorage is nil", func() {
				...
				Context("ExternalService is correctly setup", func() {
					...
					It("tests that the ExternalService is setup with the default service name", func() {
						r, err := NewReconcilerWithShims(cli, scheme, mockStatus, operatorv1.ProviderNone, &mockESClient{})
						...
						Expect(svc.Spec.ExternalName).Should(Equal("tigera-guardian.tigera-guardian.svc"))
		...
		Context("Unmanaged cluster", func() {
			Context("successful LogStorage Reconcile", func() {
				...
				It("test LogStorage reconciles successfully for elasticsearch basic license", func() {
					...
					Expect(cli.Create(ctx, render.CreateDexTLSSecret("tigera-dex.tigera-dex.svc"))).ToNot(HaveOccurred())
...
func setUpLogStorageComponents(cli client.Client, ctx context.Context, storageClass string, managementClusterConnection *operatorv1.ManagementClusterConnection) {
	...
	component := render.LogStorage(
		...
		nil, nil, false, nil, render.ElasticsearchLicenseTypeBasic,
		&corev1.ConfigMap{
